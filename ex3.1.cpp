...
void Arrow::draw_lines() const
{
    ...
    if (color().visibility())
    {
        fl_line(point(1).x, point(1).y, pl_x, pl_y);
        fl_line(point(1).x, point(1).y, pr_x, pr_y);
    }
*/
    ...
    Point arrowPoint1{
        point(1).x - unitDx * arrowHeadBoxSize - unitDy * arrowHeadBoxSize,
        point(1).y - unitDy * arrowHeadBoxSize + unitDx * arrowHeadBoxSize};
    Point arrowPoint2{
        point(1).x - unitDx * arrowHeadBoxSize + unitDy * arrowHeadBoxSize,
        point(1).y - unitDy * arrowHeadBoxSize - unitDx * arrowHeadBoxSize};

    fl_line(point(1).x, point(1).y, arrowPoint1.x, arrowPoint1.y);
    fl_line(point(1).x, point(1).y, arrowPoint2.x, arrowPoint2.y);
*/
}
